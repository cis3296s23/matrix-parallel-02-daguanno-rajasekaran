...
int main(int argc, char *argv[]) {
    ...
    //returns error if there are more than 2 arguments
    if(argc > 2){
    ...
    for (int i = 50; i <= 2000; i = i + 50) {
        ...
        a = read_matrix_from_file(argv[0]);
        b = read_matrix_from_file(argv[1]);
        ...
        if(unoptimized) {
            ...
            double Uproduct = mmult(c_calc, a, i, i, b, i, i);
        ...
        if(OMP) {
            clock_gettime(CLOCK_REALTIME, &tps);
            double OMPproduct = mmult_omp(c_calc, a, i, i, b, i, i);
            clock_gettime(CLOCK_REALTIME, &tpe);
            printf("OMP         delta: %f\n", deltaTime(&tps, &tpe));
        }
